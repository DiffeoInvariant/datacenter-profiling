...
int main(int argc, char **argv)
{
  ...
  while((nread = getline(&line,&linesize,input.file)) != -1) {
    ...
    switch (input_type) {
    case TCPACCEPT:
      ierr = create_tcpaccept_entry_bag(&accept_entry,&bag,nentry);CHKERRQ(ierr);
    ...
    case TCPCONNECT:
      ...
      ierr = create_tcpconnect_entry_bag(&connect_entry,&bag,nentry);CHKERRQ(ierr);
	...
	ignore_entry = PETSC_TRUE;
      ...
      break;
    case TCPCONNLAT:
      ierr = create_tcpconnlat_entry_bag(&connlat_entry,&bag,nentry);CHKERRQ(ierr);
	...
	ignore_entry = PETSC_TRUE;
      ...
      break;
    case TCPLIFE:
      ierr = create_tcplife_entry_bag(&life_entry,&bag,nentry);CHKERRQ(ierr);
	...
	ignore_entry = PETSC_TRUE;
      ...
      break;
    case TCPRETRANS:
      ierr = create_tcpretrans_entry_bag(&retrans_entry,&bag);CHKERRQ(ierr);
    ...
    //ierr = PetscBagView(bag,PETSC_VIEWER_STDOUT_WORLD);
  ...
  while (!buffer_empty(&buf)) {
    ...
    ierr = buffer_next(&buf);CHKERRQ(ierr);
  }
